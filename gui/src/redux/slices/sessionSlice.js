...
export const sessionSlice = createSlice({
    ...
    initialState,
    reducers: {
        addPromptCompletionPair: (state, { payload }) => {
        ...
        setIsGatheringContext: (state, { payload }) => {
        ...
        setMainEditorContentTrigger: (state, action) => {
            state.mainEditorContentTrigger = action.payload;
        },
        updateFileSymbols: (state, action) => {
            state.symbols = {
                ...state.symbols,
                ...action.payload,
            };
        },
        setContextItemsAtIndex: (state, { payload: { index, contextItems }, }) => {
            if (state.history[index]) {
                state.history[index].contextItems = contextItems;
            }
        },
        submitEditorAndInitAtIndex: (state, { payload, }) => {
            ...
            if (state.history.length && index < state.history.length) {
                ...
                if (index % 2 === 1) {
                    console.warn("Corrupted history: resubmitting at odd index, shouldn't happen");
                }
        ...
        updateHistoryItemAtIndex: (state, { payload, }) => {
            const { index, updates } = payload;
            if (index !== 0 && !state.history[index]) {
                console.error(`attempting to update history item at nonexistent index ${index}`, updates);
                return;
            }
            state.history[index] = {
                ...state.history[index],
        ...
        addContextItemsAtIndex: (state, { payload, }) => {
        ...
        setAppliedRulesAtIndex: (state, { payload, }) => {
                ...
                for (const message of action.payload) {
                    ...
                    if (lastMessage.role !== message.role ||
                        ...
                        if (message.role === "assistant" && message.toolCalls?.[0]) {
                            ...
                            historyItem.toolCallState = addToolCallDeltaToState(toolCallDelta, undefined);
                        }
                        ...
                        if (message.content) {
                            ...
                            const toolCallDelta = message.toolCalls[0];
                            const newToolCallState = addToolCallDeltaToState(toolCallDelta, lastItem.toolCallState);
        ...
        deleteSessionMetadata: (state, { payload }) => {
            // Note, should not be allowed to delete current session from chat session
            state.allSessionMetadata = state.allSessionMetadata.filter((session) => session.sessionId !== payload);
        },
        //////////////////////////////////////////////////////////////////////////////////
        addHighlightedCode: (state, { payload, }) => {
            let contextItems = state.history[state.history.length - 1].contextItems ?? [];
            contextItems = contextItems.map((item) => {
                return { ...item, editing: false };
            });
            const { relativePathOrBasename } = findUriInDirs(payload.rangeInFileWithContents.filepath, window.workspacePaths ?? []);
            const fileName = getUriPathBasename(payload.rangeInFileWithContents.filepath);
            const lineNums = `(${payload.rangeInFileWithContents.range.start.line + 1}-${payload.rangeInFileWithContents.range.end.line + 1})`;
            contextItems.push({
        ...
        updateApplyState: (state, { payload }) => {
            const applyState = state.codeBlockApplyStates.states.find((state) => state.streamId === payload.streamId);
            if (!applyState) {
        ...
        // TOOL CALL STATE
        setToolGenerated: (state, action) => {
            const toolCallState = findToolCall(state.history, action.payload.toolCallId);
            if (toolCallState) {
                toolCallState.status = "generated";
            }
        },
        updateToolCallOutput: (state, action) => {
            const toolCallState = findToolCall(state.history, action.payload.toolCallId);
            if (toolCallState) {
                toolCallState.output = action.payload.contextItems;
            }
        },
        cancelToolCall: (state, action) => {
            const toolCallState = findToolCall(state.history, action.payload.toolCallId);
            if (toolCallState) {
                toolCallState.status = "canceled";
            }
        },
        errorToolCall: (state, action) => {
            const toolCallState = findToolCall(state.history, action.payload.toolCallId);
            if (toolCallState) {
                toolCallState.status = "errored";
            }
        },
        acceptToolCall: (state, action) => {
            const toolCallState = findToolCall(state.history, action.payload.toolCallId);
            if (toolCallState) {
                toolCallState.status = "done";
            }
        },
        setToolCallCalling: (state, action) => {
            const toolCallState = findToolCall(state.history, action.payload.toolCallId);
            if (toolCallState) {
        ...
        setNewestToolbarPreviewForInput: (state, { payload, }) => {
...
function addPassthroughCases(builder, thunks) {
...
export const { updateFileSymbols, setContextItemsAtIndex, addContextItemsAtIndex, setAppliedRulesAtIndex, setInactive, streamUpdate, newSession, updateSessionTitle, addHighlightedCode, addPromptCompletionPair, setActive, submitEditorAndInitAtIndex, updateHistoryItemAtIndex, clearLastEmptyResponse, setMainEditorContentTrigger, deleteMessage, setIsGatheringContext, resetNextCodeBlockToApplyIndex, updateApplyState, abortStream, setToolCallCalling, cancelToolCall, errorToolCall, acceptToolCall, setToolGenerated, updateToolCallOutput, setMode, setAllSessionMetadata, addSessionMetadata, updateSessionMetadata, deleteSessionMetadata, setNewestToolbarPreviewForInput, setIsInEdit, } = sessionSlice.actions;
