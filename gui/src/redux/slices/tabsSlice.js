...
export const tabsSlice = createSlice({
    ...
    reducers: {
        ...
        handleSessionChange: (state, action) => {
            const { currentSessionId, currentSessionTitle, newTabId } = action.payload;
            ...
            if (activeTab.sessionId === currentSessionId) {
                state.tabs = state.tabs.map((tab) => tab.id === activeTab.id
                    ? { ...tab, title: currentSessionTitle }
                    : tab);
                return;
            }
            // Check if there's another tab with the same session ID
            const existingTabWithSession = state.tabs.find((tab) => tab.sessionId === currentSessionId);
...
export const { setTabs, updateTab, addTab, removeTab, setActiveTab, handleSessionChange, } = tabsSlice.actions;
export default tabsSlice.reducer;
//# sourceMappingURL=tabsSlice.js.map