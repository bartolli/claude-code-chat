...
export const profilesSlice = createSlice({
    ...
    reducers: {
        setSelectedProfile: (state, { payload }) => {
            ...
            const currentOrg = state.organizations.find((o) => o.id === state.selectedOrganizationId);
            if (currentOrg) {
        ...
        setSelectedOrgId: (state, { payload }) => {
        ...
        initializeProfilePreferences: (state, action) => {
            const { profileId, defaultSlashCommands = [] } = action.payload;
            const defaultSlashCommandNames = defaultSlashCommands.map((cmd) => cmd.name);
            ...
            if (!state.preferencesByProfileId[profileId]) {
                state.preferencesByProfileId[profileId] = {
                    bookmarkedSlashCommands: defaultSlashCommandNames.slice(0, DEFAULT_SLASH_COMMANDS_BOOKMARKS_COUNT),
                };
            }
        },
        bookmarkSlashCommand: (state, action) => {
        ...
        unbookmarkSlashCommand: (state, action) => {
            const { commandName } = action.payload;
            ...
            preferences.bookmarkedSlashCommands =
                preferences.bookmarkedSlashCommands.filter((cmd) => cmd !== commandName);
        },
    },
    selectors: {
        selectSelectedProfile: (state) => {
            const selectedOrg = state.organizations.find((org) => org.id === state.selectedOrganizationId);
            if (selectedOrg) {
                const profile = selectedOrg.profiles.find((profile) => profile.id === state.selectedProfileId);
                return profile ?? null;
            }
            else {
                return null;
...
export const { setSelectedProfile, bookmarkSlashCommand, unbookmarkSlashCommand, initializeProfilePreferences, setOrganizations, setSelectedOrgId, } = profilesSlice.actions;
export const { selectSelectedProfile, selectBookmarkedSlashCommands, selectPreferencesByProfileId, selectCurrentOrg, } = profilesSlice.selectors;
export const { reducer: profilesReducer } = profilesSlice;
//# sourceMappingURL=profilesSlice.js.map