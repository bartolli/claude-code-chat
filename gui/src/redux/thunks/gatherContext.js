...
export const gatherContext = createAsyncThunk("chat/gatherContext", async ({ modifiers, editorState, promptPreamble }, { dispatch, extra, getState }) => {
    ...
    let [selectedContextItems, selectedCode, content, slashCommandWithInput] = await resolveEditorContent({
        editorState,
        modifiers,
    ...
    if (!modifiers.noContext) {
        ...
        const currentFileResponse = await extra.ideMessenger.request("context/getContextItems", {
            name: "currentFile",
        ...
        if (currentFileResponse.status === "success") {
            ...
            if (items.length > 0) {
                ...
                if (uri &&
                    !selectedContextItems.find((item) => item.uri?.value && URI.equal(item.uri.value, uri))) {
