...
import { clearCodeToEdit, INITIAL_EDIT_APPLY_STATE, setPreviousModeEditorContent, setReturnToModeAfterEdit, updateEditStateApplyState, } from "../slices/editState";
import { newSession, setActive, setIsInEdit, setMainEditorContentTrigger, setMode, } from "../slices/sessionSlice";
import { loadLastSession, saveCurrentSession } from "./session";
import { streamThunkWrapper } from "./streamThunkWrapper";
export const streamEditThunk = createAsyncThunk("chat/streamResponse", async ({ editorState, codeToEdit }, { dispatch, extra }) => {
...
export const exitEdit = createAsyncThunk("edit/exit", async ({ goToMode, openNewSession }, { dispatch, extra, getState }) => {
    const state = getState();
    const codeToEdit = state.editModeState.codeToEdit;
    const isInEdit = state.session.isInEdit;
    const previousModeEditorContent = state.editModeState.previousModeEditorContent;
    if (!isInEdit) {
        return;
    }
...
export const enterEdit = createAsyncThunk("edit/enter", async ({ returnToMode, editorContent }, { dispatch, extra, getState }) => {
    const state = getState();
    const isInEdit = state.session.isInEdit;
    if (isInEdit) {
        return;
