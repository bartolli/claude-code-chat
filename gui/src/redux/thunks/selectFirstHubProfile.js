 ...
 */
export const selectFirstHubProfile = createAsyncThunk("selectFirstHubProfile", async (_, { dispatch, extra, getState }) => {
    let attempts = 0;
    const maxAttempts = 3;
    const findAndSelectProfile = () => {
        const state = getState();
        const currentOrgProfiles = state.profiles.organizations.find((org) => org.id === (state.profiles.selectedOrganizationId ?? "personal"));
        const firstHubProfile = currentOrgProfiles?.profiles.find((profile) => profile.profileType === "platform");
        if (firstHubProfile) {
...
//# sourceMappingURL=selectFirstHubProfile.js.map