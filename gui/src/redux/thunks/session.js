...
import { deleteSessionMetadata, newSession, setAllSessionMetadata, updateSessionMetadata, } from "../slices/sessionSlice";
...
export async function getSession(ideMessenger, id) {
...
export const refreshSessionMetadata = createAsyncThunk("session/refreshMetadata", async ({ offset, limit }, { dispatch, extra }) => {
...
export const deleteSession = createAsyncThunk("session/delete", async (id, { getState, dispatch, extra }) => {
...
export const updateSession = createAsyncThunk("session/update", async (session, { extra, dispatch }) => {
...
*/
export const loadSession = createAsyncThunk("session/load", async ({ sessionId, saveCurrentSession: save }, { extra, dispatch }) => {
    if (save) {
        const result = await dispatch(saveCurrentSession({
...
export const loadLastSession = createAsyncThunk("session/loadLast", async ({ saveCurrentSession }, { extra, dispatch, getState }) => {
    const state = getState();
...
export const saveCurrentSession = createAsyncThunk("session/saveCurrent", async ({ openNewSession, generateTitle }, { dispatch, extra, getState }) => {
    const state = getState();
    ...
    if (title === NEW_SESSION_TITLE) {
        ...
        if (!state.config.config?.disableSessionTitles && selectedChatModel) {
            ...
            if (assistantResponse && generateTitle) {
                try {
                    const result = await extra.ideMessenger.request("chatDescriber/describe", {
    ...
    if (!title.length) {
        const metadata = getState().session.allSessionMetadata.find((m) => m.sessionId === state.session.id);
        if (metadata?.title) {
