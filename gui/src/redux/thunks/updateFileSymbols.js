...
export function getContextItemsFromHistory(historyItems, priorToIndex) {
    const pastHistoryItems = historyItems.filter((_, i) => i <= (priorToIndex ?? historyItems.length - 1));
    const pastNormalContextItems = pastHistoryItems.flatMap((item) => {
        return (item.contextItems?.filter((item) => item.uri?.type === "file" && item.uri?.value) ?? []);
    });
    const pastToolbarContextItems = pastHistoryItems
        .filter((item) => item.editorState && Array.isArray(item.editorState.content))
        .flatMap((item) => item.editorState.content)
        .filter((content) => content?.type === CodeBlock.name &&
...
*/
export const updateFileSymbolsFromFiles = createAsyncThunk("symbols/updateFromContextItems", async (filepaths, { dispatch, extra, getState }) => {
    try {
        // Get unique file uris from context items
        const uniqueUris = Array.from(new Set(filepaths));
        // Update symbols for those files
        if (uniqueUris.length > 0) {
            const result = await extra.ideMessenger.request("context/getSymbolsForFiles", { uris: uniqueUris });
...
*/
export const updateFileSymbolsFromHistory = createAsyncThunk("symbols/updateFromHistory", async (_, { dispatch, extra, getState }) => {
    try {
        ...
        const uniqueUris = new Set(contextItems
        ...
        if (uriArray.length > 0) {
            const result = await extra.ideMessenger.request("context/getSymbolsForFiles", { uris: uriArray });
            if (result.status === "error") {
                throw new Error(result.error);
            }
            dispatch(updateFileSymbols(result.content));
...
//# sourceMappingURL=updateFileSymbols.js.map