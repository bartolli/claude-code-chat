...
import { acceptToolCall, errorToolCall, setToolCallCalling, updateToolCallOutput, } from "../slices/sessionSlice";
import { streamResponseAfterToolCall } from "./streamResponseAfterToolCall";
export const callCurrentTool = createAsyncThunk("chat/callTool", async (_, { dispatch, extra, getState }) => {
    ...
    if (CLIENT_TOOLS_IMPLS.find((toolName) => toolName === toolCallState.toolCall.function.name)) {
        // Tool is called on client side
        const { output: clientToolOuput, respondImmediately, errorMessage: clientToolError, } = await callClientTool(toolCallState, {
            dispatch,
            ideMessenger: extra.ideMessenger,
            streamId: state.session.codeBlockApplyStates.states.find((state) => state.toolCallId && state.toolCallId === toolCallState.toolCallId)?.streamId,
            getState,
        });
    ...
    if (streamResponse) {
        ...
        const wrapped = await dispatch(streamResponseAfterToolCall({
            toolCallId,
        }));
        unwrapResult(wrapped);
    }
});
//# sourceMappingURL=callCurrentTool.js.map