...
export function isMetaEquivalentKeyPressed({ metaKey, ctrlKey, }) {
...
export function isPrerelease() {
    ...
    if (!extensionVersion) {
        console.warn(`Could not find extension version in local storage, assuming it's a prerelease`);
        return true;
    }
 ...
 */
export function updatedObj(old, pathToValue) {
 ...
 */
export function getBaseSystemMessage(modelDetails, mode) {
    let baseChatOrAgentSystemMessage;
    if (mode === "agent") {
