...
export const editToolImpl = async (args, toolCallId, extras) => {
    if (!extras.streamId) {
        throw new Error("Invalid apply state");
    }
    const firstUriMatch = await resolveRelativePathInDir(args.filepath, extras.ideMessenger.ide);
    if (!firstUriMatch) {
...
//# sourceMappingURL=editImpl.js.map