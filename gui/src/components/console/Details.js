...
function renderCell(children) {
    return (<div className="border-0 border-r-2 border-solid border-[color:var(--vscode-panel-border)] pl-2 pr-2 text-sm">
      {children}
    </div>);
}
/**
 ...
 */
function Cell({ label, value, format, }) {
    return renderCell(value != undefined ? `${label}: ${format ? format(value) : value}` : "");
}
/**
 * A cell of statistics at the top of the details view with custom content
 */
function CustomCell({ children }) {
    return renderCell(children);
}
function formatSeconds(milliseconds) {
    return (milliseconds / 1000).toFixed(2) + "s";
}
export default function Details({ interaction }) {
    ...
    return (<div className="m-0 flex min-w-0 flex-1 shrink grow flex-col">
      <div className="shrink-0 text-base">
        ...
        <div className="columns-3 gap-0 border-0 border-b-2 border-solid border-[color:var(--vscode-panel-border)] p-0">
          <Cell label="Total Time" value={summary.totalTime} format={formatSeconds}></Cell>
          <Cell label="To First Token" value={summary.toFirstToken} format={formatSeconds}></Cell>
          <Cell label="Tokens/s" value={summary.tokensPerSecond} format={(v) => v.toFixed(1)}></Cell>
        </div>
      </div>
      <div ref={scrollTop} className="grow overflow-auto" onScroll={handleScroll}>
