...
const Result = memo(function Result({ result, prevResult }) {
    switch (result.kind) {
        case "chunk":
        ...
        case "message":
            switch (result.message.role) {
                case "assistant":
                case "thinking":
                    const includeRole = !(prevResult?.kind === "message" &&
                ...
                default:
                    ...
                    return (<div className="border-[color:var(--vscode-panel-border) border-2 border-solid p-1">
