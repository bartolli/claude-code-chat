 ...
 */
export const CodeBlockPreview = ({ node, deleteNode, selected, }) => {
    ...
    const handleTitleClick = (e) => {
        ...
        if (item.id.providerTitle === "file" && item.uri?.value) {
            ...
            ideMessenger.ide.showLines(rif.filepath, rif.range.start.line, rif.range.end.line);
        }
        else {
    ...
    return (<NodeViewWrapper>
      <ExpandableToolbarPreview isSelected={selected} title={item.name} icon={<FileIcon height="16px" width="16px" filename={item.name}/>} inputId={inputId} itemId={item.id.itemId} onDelete={() => deleteNode()} borderColor={selected ? vscBadgeBackground : undefined} onTitleClick={handleTitleClick}>
        {!content ? null : (<StyledMarkdownPreview source={`${fence}${getMarkdownLanguageTagForFile(item.name)} ${item.description}\n${content}\n${fence}`}/>)}
      </ExpandableToolbarPreview>
    </NodeViewWrapper>);
};
//# sourceMappingURL=CodeBlockPreview.js.map