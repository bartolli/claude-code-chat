...
export function TipTapEditor(props) {
    ...
    const insertCharacterWithWhitespace = useCallback((char) => {
        if (!editor) {
            return;
        }
    ...
    return (<InputBoxDiv onFocus={handleFocus} onBlur={handleBlur} onKeyDown={handleKeyDown} onKeyUp={handleKeyUp} className={shouldHideToolbar ? "cursor-default" : "cursor-text"} onClick={() => {
            editor?.commands.focus();
        }} onDragOver={(event) => {
      ...
      <div className="px-2.5 pb-1 pt-2">
        <EditorContent className={`scroll-container overflow-y-scroll ${props.isMainInput ? "max-h-[70vh]" : ""}`} spellCheck={false} editor={editor} onClick={(event) => {
            event.stopPropagation();
        }}/>
        <InputToolbar isMainInput={props.isMainInput} toolbarOptions={props.toolbarOptions} activeKey={activeKey} hidden={shouldHideToolbar && !props.isMainInput} onAddContextItem={() => insertCharacterWithWhitespace("@")} onEnter={onEnterRef.current} onImageFileSelected={(file) => {
            void handleImageFile(ideMessenger, file).then((result) => {
                if (!editor) {
                    return;
                }
      ...
      {showDragOverMsg &&
            modelSupportsImages(defaultModel?.provider || "", defaultModel?.model || "", defaultModel?.title, defaultModel?.capabilities) && (<DragOverlay show={showDragOverMsg} setShow={setShowDragOverMsg}/>)}
