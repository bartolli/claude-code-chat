 ...
 */
export function useMainEditorWebviewListeners({ editor, onEnterRef, dispatch, historyLength, inputId, editorFocusedRef, }) {
    const activeContextProviders = useAppSelector((state) => state.config.config.contextProviders);
    const useCurrentFileAsContext = useAppSelector((state) => state.config.config.experimental?.useCurrentFileAsContext);
    const isInEdit = useAppSelector((state) => state.session.isInEdit);
    useWebviewListener("isContinueInputFocused", async () => {
        return !!editorFocusedRef.current;
    }, [editorFocusedRef]);
    useWebviewListener("userInput", async (data) => {
        if (!editor)
            return;
    ...
    useWebviewListener("focusContinueInput", async () => {
    ...
    useWebviewListener("focusContinueInputWithoutClear", async () => {
        setTimeout(() => {
            editor?.commands.focus("end");
        }, 20);
    }, [editor]);
    useWebviewListener("focusContinueInputWithNewSession", async () => {
        await dispatch(saveCurrentSession({
            openNewSession: true,
            generateTitle: true,
        }));
    ...
    useWebviewListener("highlightedCode", async (data) => {
        if (!editor)
            return;
        const contextItem = rifWithContentsToContextItem(data.rangeInFileWithContents);
        let index = 0;
        ...
        editor
            ...
            .run();
        dispatch(setNewestToolbarPreviewForInput({
            inputId,
    ...
    useWebviewListener("focusContinueSessionId", async (data) => {
        if (!data.sessionId)
            return;
    ...
    useWebviewListener("newSession", async () => {
        ...
        if (!editor || isInEdit || !useCurrentFileAsContext)
            return;
...
//# sourceMappingURL=useMainEditorWebviewListeners.js.map