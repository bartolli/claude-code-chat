 ...
 */
export async function resolveEditorContent({ editorState, modifiers, ideMessenger, defaultContextProviders, availableSlashCommands, dispatch, }) {
    const { parts, contextItemAttrs, selectedCode, slashCommandName } = processEditorContent(editorState);
    const slashCommandWithInput = processSlashCommand(slashCommandName, availableSlashCommands, parts);
 ...
 */
function processEditorContent(editorState) {
    const contextItemAttrs = [];
    const selectedCode = [];
    let slashCommandName;
    const parts = (editorState?.content || []).reduce((parts, p) => {
        switch (p.type) {
            ...
            case CodeBlock.name: {
                ...
                const contextItem = p.attrs.item;
 ...
 */
function processSlashCommand(slashCommandName, availableSlashCommands, parts) {
    if (!slashCommandName)
        return;
    const command = availableSlashCommands.find((c) => c.name === slashCommandName);
    if (!command)
        return;
    const lastTextIndex = findLastIndex(parts, (part) => part.type === "text");
    const lastTextPart = parts[lastTextIndex];
 ...
 */
async function gatherContextItems({ contextItemAttrs, modifiers, ideMessenger, defaultContextProviders, parts, selectedCode, }) {
    let contextItems = [];
    ...
    const defaultContextItems = await Promise.all(defaultContextProviders.map(async (provider) => {
        const result = await ideMessenger.request("context/getContextItems", {
            name: provider.name,
...
function resvolePromptBlock(p) {
    return p.attrs?.item.name;
}
function resolveParagraph(p) {
    const contextItems = [];
...
//# sourceMappingURL=resolveEditorContent.js.map