...
import { getContextProviderDropdownOptions, getSlashCommandDropdownOptions, } from "./getSuggestion";
import { handleImageFile } from "./imageUtils";
export function getPlaceholderText(placeholder, historyLength) {
    if (placeholder) {
        return placeholder;
    }
 ...
 */
export function hasValidEditorContent(json) {
    // Check for prompt or code blocks
    const hasPromptOrCodeBlock = json.content?.some((c) => c.type === PromptBlock.name || c.type === CodeBlock.name);
...
export function createEditorConfig(options) {
    ...
    const availableSlashCommandsRef = useUpdatingRef(props.availableSlashCommands);
    const { prevRef, nextRef, addRef } = useInputHistory(props.historyKey);
    const enterSubmenu = async (editor, providerId) => {
    ...
    const editor = useEditor({
        extensions: [
                                ...
                                paste(view, event) {
                                    ...
                                    if (items) {
                                        for (const item of items) {
                                            ...
                                            file &&
                                                void modelSupportsImages(model.provider, model.model, model.title, model.capabilities) &&
            ...
            Placeholder.configure({
                placeholder: getPlaceholderText(props.placeholder, historyLengthRef.current),
            }),
            Paragraph.extend({
                addKeyboardShortcuts() {
                    return {
                        ...
                        ArrowDown: () => {
                            if (this.editor.state.selection.anchor <
            ...
            Text,
            Mention.configure({
                suggestion: getContextProviderDropdownOptions(availableContextProvidersRef, getSubmenuContextItemsRef, enterSubmenu, onClose, onOpen, inSubmenuRef, ideMessenger),
            }),
            SlashCommand.configure({
                suggestion: getSlashCommandDropdownOptions(availableSlashCommandsRef, onClose, onOpen, ideMessenger, dispatch, props.inputId),
            }),
            PromptBlock,
            CodeBlock,
        ],
    ...
    const onEnterRef = useUpdatingRef((modifiers) => {
        if (!editor) {
            return;
        }
