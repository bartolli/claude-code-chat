...
`;
function ThinkingBlockPeek({ content, redactedThinking, index, prevItem, inProgress, signature, tokens, }) {
    const [open, setOpen] = useState(false);
    const [startTime, setStartTime] = useState(null);
    const [elapsedTime, setElapsedTime] = useState("");
    const duplicateRedactedThinkingBlock = prevItem &&
    ...
    return duplicateRedactedThinkingBlock ? null : (<div className="thread-message">
      <div className="" style={{ backgroundColor: vscBackground }}>
        <div className="flex items-center justify-start pl-2 text-xs text-gray-300" data-testid="thinking-block-peek">
          <SpoilerButton onClick={() => setOpen((prev) => !prev)}>
            <ButtonContent>
              {inProgress ? (<ThinkingText>
                  {redactedThinking ? "Redacted Thinking" : "Thinking"}
                </ThinkingText>) : redactedThinking ? ("Redacted Thinking") : ("Thought" +
        ...
        <div className={`ml-2 mt-2 overflow-y-auto transition-none duration-300 ease-in-out ${open ? "mb-2 mt-5 opacity-100" : "max-h-0 border-0 opacity-0"}`} style={{
            borderLeft: open && !redactedThinking
                ? "2px solid var(--vscode-input-border, #606060)"
                : "none",
        }}>
          {redactedThinking ? (<div className="pl-4 text-xs text-gray-400">
...
//# sourceMappingURL=ThinkingBlockPeek.js.map