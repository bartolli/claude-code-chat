    ...
    return (<div className="flex flex-col gap-1">
      <div className="flex flex-col overflow-y-auto overflow-x-hidden pr-2">
        {sortedConfigDocs.map((docConfig) => {
            return (<div key={docConfig.doc.startUrl} className="flex items-center gap-2">
              <div className="flex-grow">
                <DocsIndexingStatus docFromYaml={docConfig.docFromYaml} docConfig={docConfig.doc}/>
              </div>
            </div>);
        })}
      </div>
...
//# sourceMappingURL=DocsSection.js.map