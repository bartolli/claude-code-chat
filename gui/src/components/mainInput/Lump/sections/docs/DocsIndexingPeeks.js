...
function DocsIndexingPeek({ status }) {
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const progressPercentage = useMemo(() => {
        return Math.min(100, Math.max(0, status.progress * 100));
    }, [status.progress]);
    return (<div className="text-lightgray flex cursor-pointer flex-row items-center gap-2 rounded-md px-1 hover:bg-gray-700/10" onClick={() => {
            // navigate("/more"); TODO
...
function DocsIndexingPeekList({ statuses }) {
    return (<div className="border-vsc-input-border mt-2 flex flex-col border-0 border-t border-solid pt-2">
...
//# sourceMappingURL=DocsIndexingPeeks.js.map