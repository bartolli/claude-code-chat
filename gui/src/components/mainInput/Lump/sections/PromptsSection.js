...
import { BookmarkIcon as BookmarkOutline, PencilIcon, } from "@heroicons/react/24/outline";
 ...
 */
function PromptRow({ prompt, isBookmarked, setIsBookmarked, onEdit, }) {
    ...
    return (<div className="hover:bg-list-active hover:text-list-active-foreground flex items-center justify-between gap-3 rounded-md px-2 py-1 hover:cursor-pointer" onClick={handlePromptClick} style={{
            fontSize: fontSize(-3),
        }}>
      ...
      <div className="flex items-center gap-2">
        <PencilIcon className="h-3 w-3 cursor-pointer text-gray-400 hover:brightness-125" onClick={handleEditClick}/>
        <div onClick={handleBookmarkClick} className="cursor-pointer pt-0.5 text-gray-400 hover:brightness-125">
          {isBookmarked ? (<BookmarkSolid className="h-3 w-3"/>) : (<BookmarkOutline className="h-3 w-3"/>)}
        </div>
      </div>
    </div>);
}
...
export function PromptsSection() {
    ...
    const slashCommands = useAppSelector((state) => state.config.config.slashCommands ?? []);
    ...
    const sortedCommands = useMemo(() => {
        const promptsWithSlug = structuredClone(slashCommands);
    ...
    return (<div className="flex flex-col">
      {sortedCommands.map((prompt) => (<PromptRow key={prompt.name} prompt={prompt} isBookmarked={isCommandBookmarked(prompt.name)} setIsBookmarked={() => toggleBookmark(prompt)} onEdit={() => handleEdit(prompt)}/>))}
      <ExploreBlocksButton blockType="prompts"/>
    </div>);
}
//# sourceMappingURL=PromptsSection.js.map