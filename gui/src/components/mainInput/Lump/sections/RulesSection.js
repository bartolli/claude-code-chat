...
import { ArrowsPointingOutIcon, EyeIcon, PencilIcon, } from "@heroicons/react/24/outline";
import { DEFAULT_AGENT_SYSTEM_MESSAGE, DEFAULT_AGENT_SYSTEM_MESSAGE_URL, DEFAULT_CHAT_SYSTEM_MESSAGE, DEFAULT_CHAT_SYSTEM_MESSAGE_URL, } from "core/llm/constructMessages";
import { useContext, useMemo } from "react";
import { useAuth } from "../../../../context/Auth";
import { IdeMessengerContext } from "../../../../context/IdeMessenger";
import { useAppDispatch, useAppSelector } from "../../../../redux/hooks";
import { DEFAULT_RULE_SETTING, setDialogMessage, setShowDialog, toggleRuleSetting, } from "../../../../redux/slices/uiSlice";
...
const RuleCard = ({ rule }) => {
    ...
    const policy = useAppSelector((state) => rule.name
    ...
    function onClickExpand() {
        ...
        dispatch(setDialogMessage(<div className="max-h-4/5 p-4">
          <h3>{title}</h3>
    ...
    return (<div className={`border-border flex flex-col rounded-sm px-2 py-1.5 transition-colors ${isDisabled ? "opacity-50" : ""}`}>
      <div className="flex flex-col">
        <div className="flex flex-row justify-between gap-1">
          <span className={`line-clamp-2 ${isDisabled ? "text-gray-400" : "text-vsc-foreground"}`} style={{
            fontSize: smallFont,
        }}>
            {title}
          </span>
          <div className="flex flex-row items-center gap-2">
            {rule.name && policy && (<div className="flex cursor-pointer flex-row items-center justify-end gap-1 px-2 py-0.5">
                <Switch isToggled={policy === "on"} onToggle={() => handleTogglePolicy()} size={10} text=""/>
              </div>)}
            ...
            rule.source === "default-agent" ? (<HeaderButtonWithToolTip onClick={handleOpen} text="View">
                  <EyeIcon className="h-3 w-3 text-gray-400"/>
                </HeaderButtonWithToolTip>) : (<HeaderButtonWithToolTip onClick={handleOpen} text="Edit">
        ...
        {rule.globs ? (<div style={{
                fontSize: tinyFont,
            }} className="mt-1.5 flex flex-col gap-1">
            <span className="italic">Applies to files</span>
            <code className={`line-clamp-1 px-1 py-0.5 ${isDisabled ? "text-gray-500" : "text-gray-400"}`}>
              {rule.globs}
            </code>
          </div>) : null}
      </div>
...
export function RulesSection() {
    ...
    const sortedRules = useMemo(() => {
        ...
        if (selectedProfile?.rawYaml) {
            try {
                ...
                console.error("Rules notch section: failed to parse selected profile", e);
            }
        }
        if (mode === "agent") {
    ...
    return (<div className="flex flex-col gap-3">
      {sortedRules.map((rule, index) => (<RuleCard key={index} rule={rule}/>))}
      <ExploreBlocksButton blockType="rules"/>
    </div>);
}
//# sourceMappingURL=RulesSection.js.map