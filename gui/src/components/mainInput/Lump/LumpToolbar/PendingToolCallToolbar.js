...
import { getAltKeyLabel, getFontSize, getMetaKeyLabel, isJetBrains, } from "../../../../util";
...
export function PendingToolCallToolbar() {
    ...
    return (<Container>
      ...
      <div className="flex gap-2 pb-0.5">
        <StopButton className="text-description" onClick={() => dispatch(cancelCurrentToolCall())} data-testid="reject-tool-call-button">
          {/* JetBrains overrides cmd+backspace, so we have to use another shortcut */}
          {jetbrains ? getAltKeyLabel() : getMetaKeyLabel()} ⌫ Cancel
        </StopButton>
        <EnterButton isPrimary={true} className="text-description" onClick={() => dispatch(callCurrentTool())} data-testid="accept-tool-call-button">
          {getMetaKeyLabel()} ⏎ Continue
        </EnterButton>
      </div>
    </Container>);
}
//# sourceMappingURL=PendingToolCallToolbar.js.map