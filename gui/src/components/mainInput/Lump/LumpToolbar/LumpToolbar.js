...
export function LumpToolbar() {
    ...
    const editApplyState = useAppSelector((state) => state.editModeState.applyState);
    const applyStates = useAppSelector((state) => state.session.codeBlockApplyStates.states);
    ...
    if (pendingApplyStates.length > 0) {
        return (<PendingApplyStatesToolbar pendingApplyStates={pendingApplyStates}/>);
    }
    return <BlockSettingsTopToolbar />;
}
//# sourceMappingURL=LumpToolbar.js.map