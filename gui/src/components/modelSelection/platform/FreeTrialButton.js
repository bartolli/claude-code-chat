...
function FreeTrialProgressBars({ freeTrialStatus, }) {
    ...
    return (<>
      <ProgressBar label="Autocomplete usage" current={autocompleteUsage.current ?? 0} total={autocompleteUsage.total}/>

      <ProgressBar label="Chat usage" current={chatUsage.current ?? 0} total={chatUsage.total}/>
    </>);
}
export default function FreeTrialButton({ freeTrialStatus, }) {
    const smallFont = useFontSize(-3);
    const tinyFont = useFontSize(-4);
    const ideMessenger = useContext(IdeMessengerContext);
    return (<Listbox>
      <div className="relative">
        <ListboxButton className="text-description border-none bg-transparent hover:brightness-125" style={{ fontSize: fontSize(-3) }}>
        ...
        <Transition>
          <ListboxOptions className="pb-0">
            ...
            await ideMessenger.request("controlPlane/openUrl", {
              ...
              {!freeTrialStatus ? (<div className="mb-4 flex items-center justify-center py-8">
                  <span className="text-description">
                    Loading trial usage...
                  </span>
                </div>) : (<FreeTrialProgressBars freeTrialStatus={freeTrialStatus}/>)}

              <div className="mt-4 flex gap-2">
                <SecondaryButton className="flex-1" onClick={async () => {
            await ideMessenger.request("controlPlane/openUrl", {
                path: "setup-models",
                orgSlug: undefined,
            });
...
//# sourceMappingURL=FreeTrialButton.js.map