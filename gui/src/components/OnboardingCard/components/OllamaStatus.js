...
export function OllamaStatus({ isOllamaConnected }) {
    const ideMessenger = useContext(IdeMessengerContext);
    const [status, setStatus] = useState(OllamaConnectionStatuses.WaitingToDownload);
    useEffect(() => {
    ...
    switch (status) {
        case OllamaConnectionStatuses.WaitingToDownload:
            return (<StyledActionButton onClick={onClickDownload}>
        ...
        case OllamaConnectionStatuses.Connected:
            return (<OllamaCompletedStep text="Ollama is running at http://localhost:11434"/>);
    }
}
//# sourceMappingURL=OllamaStatus.js.map