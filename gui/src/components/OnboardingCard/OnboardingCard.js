...
export function OnboardingCard({ isDialog, showFreeTrialExceededAlert, }) {
    ...
    if (activeTab) {
        return (<ReusableCard showCloseButton={!isDialog && !!config.modelsByRole.chat.length} onClose={close} testId="onboarding-card">
        {showFreeTrialExceededAlert && (<div className="mb-3 mt-4">
            <Alert>
        ...
        <OnboardingCardTabs activeTab={activeTab} onTabClick={setActiveTab} showFreeTrialExceededAlert/>
        {renderTabContent()}
      </ReusableCard>);
    }
    return (<ReusableCard showCloseButton={!isDialog && !!config.modelsByRole.chat.length} onClose={close}>
      <div className="flex h-full w-full items-center justify-center">
        <OnboardingCardLanding onSelectConfigure={() => setActiveTab(OnboardingModes.API_KEY)} isDialog={isDialog}/>
      </div>
    </ReusableCard>);
}
//# sourceMappingURL=OnboardingCard.js.map