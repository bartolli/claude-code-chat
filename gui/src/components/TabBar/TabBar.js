...
import { addTab, handleSessionChange, removeTab, setActiveTab, setTabs, } from "../../redux/slices/tabsSlice";
...
const Tab = styled.div `
  ...
  max-width: 150px;
  height: 25px;
  background-color: ${(props) => props.isActive ? tabSelectedBackgroundVar : tabBackgroundVar};
  color: ${(props) => props.isActive ? tabSelectedForegroundVar : tabForegroundVar};
  cursor: pointer;
  ...
  border-top: ${(props) => props.isActive ? `1px solid ${tabAccentVar}` : `1px solid ${tabBorderVar}`};
  &:first-child {
    border-left: none;
  }
  & + & {
  ...
  &:hover {
    background-color: ${(props) => props.isActive ? tabSelectedBackgroundVar : tabHoverBackgroundVar};
    color: ${(props) => props.isActive ? tabSelectedForegroundVar : tabHoverForegroundVar};
  }
`;
...
const TabBarSpace = styled.div `
  ...
  background-color: ${tabBackgroundVar};
`;
export const TabBar = React.forwardRef((_, ref) => {
    const dispatch = useDispatch();
    const currentSessionId = useSelector((state) => state.session.id);
    const currentSessionTitle = useSelector((state) => state.session.title);
    const hasHistory = useSelector((state) => state.session.history.length > 0);
    ...
    const handleNewTab = async () => {
        ...
        if (hasHistory) {
            await dispatch(saveCurrentSession({ openNewSession: false, generateTitle: true }));
        }
        dispatch(newSession());
        dispatch(addTab({
            id: generateId(),
    ...
    const handleTabClose = async (id) => {
        ...
        if (isClosingActive) {
            ...
            if (filtered.length) {
                ...
                dispatch(setTabs(filtered.map((tab, i) => ({
                    ...tab,
    ...
    return (<TabBarContainer ref={ref} style={{
            display: tabs.length === 1 ? "none" : "flex",
        }}>
      {tabs.map((tab) => (<Tab key={tab.id} isActive={tab.isActive} onClick={() => handleTabClick(tab.id)}>
