...
import { defaultBorderRadius, vscBackground, vscEditorBackground, vscForeground, } from "..";
...
function getCodeChildrenContent(children) {
...
const StyledMarkdownPreview = memo(function StyledMarkdownPreview(props) {
    ...
    const pastFileInfo = useMemo(() => {
        ...
        const rifs = pastContextItems.map((item) => ctxItemToRifWithContents(item, true));
    ...
    const codeblockStreamIds = useRef([]);
    useEffect(() => {
        ...
        rehypeReactOptions: {
            components: {
                a: ({ ...aProps }) => {
                    ...
                    return (<>
              <a href={aProps.href} target="_blank" className="hover:underline" data-tooltip-id={tooltipId}>
                {aProps.children}
              </a>
              <ToolTip id={tooltipId} place="top" className="m-0 p-0">
                ...
                pre: ({ ...preProps }) => {
                    ...
                    return (<StepContainerPreToolbar codeBlockContent={codeBlockContent} itemIndex={itemIndexRef.current} codeBlockIndex={codeBlockIndex} language={language} relativeFilepath={relativeFilePath} isLastCodeblock={isLastCodeblock} range={range} codeBlockStreamId={codeblockStreamIds.current[codeBlockIndex]} expanded={props.expandCodeblocks} disableManualApply={props.disableManualApply}>
    ...
    return (<StyledMarkdown contentEditable="false" fontSize={getFontSize()} whiteSpace={codeWrapState} bgColor={props.useParentBackgroundColor ? "" : vscBackground}>
      {reactContent}
    </StyledMarkdown>);
});
