...
import { defaultBorderRadius, vscCommandCenterInactiveBorder, vscEditorBackground, } from "../..";
...
const ToolbarDiv = styled.div `
  ...
  padding: 4px 6px;
  margin: 0;
  border-bottom: ${({ isExpanded }) => isExpanded ? `1px solid ${vscCommandCenterInactiveBorder}` : "inherit"};
`;
export function StepContainerPreToolbar({ codeBlockContent, language, relativeFilepath, itemIndex, codeBlockIndex, isLastCodeblock, codeBlockStreamId, range, children, expanded, disableManualApply, }) {
    const ideMessenger = useContext(IdeMessengerContext);
    const history = useAppSelector((state) => state.session.history);
    const [isExpanded, setIsExpanded] = useState(expanded ?? true);
    const [relativeFilepathUri, setRelativeFilepathUri] = useState(null);
    const fileExistsInput = useMemo(() => (relativeFilepathUri ? { filepath: relativeFilepathUri } : null), [relativeFilepathUri]);
    const { result: fileExists, refresh: refreshFileExists, isLoading: isLoadingFileExists, } = useIdeMessengerRequest("fileExists", fileExistsInput);
    const nextCodeBlockIndex = useAppSelector((state) => state.session.codeBlockApplyStates.curIndex);
    const applyState = useAppSelector((state) => selectApplyStateByStreamId(state, codeBlockStreamId));
     ...
     */
    const [appliedFileUri, setAppliedFileUri] = useState(undefined);
    const isNextCodeBlock = nextCodeBlockIndex === codeBlockIndex;
    const hasFileExtension = relativeFilepath && /\.[0-9a-z]+$/i.test(relativeFilepath);
    ...
    useWebviewListener("applyCodeFromChat", async () => onClickApply(), [isNextCodeBlock, codeBlockContent], !isNextCodeBlock);
    useEffect(() => {
        const getRelativeFilepathUri = async () => {
            if (relativeFilepath) {
                const resolvedUri = await inferResolvedUriFromRelativePath(relativeFilepath, ideMessenger.ide);
    ...
    async function onClickApply() {
        ...
        if (!fileUri) {
            ideMessenger.ide.showToast("error", "Could not resolve filepath to apply changes");
            return;
        }
    ...
    async function handleDiffAction(action) {
        const filepath = await getFileUriToApplyTo();
        if (!filepath) {
            ideMessenger.ide.showToast("error", `Could not resolve filepath to ${action} changes`);
            return;
        }
    ...
    async function onClickFilename() {
        ...
        if (relativeFilepath) {
            const filepath = await inferResolvedUriFromRelativePath(relativeFilepath, ideMessenger.ide);
            ideMessenger.post("showFile", {
                filepath,
            });
        }
    ...
    const renderActionButtons = () => {
        const isPendingToolCall = currentToolCallApplyState &&
        ...
        if (isGeneratingCodeBlock || isPendingToolCall) {
            ...
            if (isGeneratingCodeBlock) {
                return (<span className="text-lightgray inline-flex items-center gap-2 text-right">
        ...
        if (fileExists || !relativeFilepath) {
            return (<ApplyActions disableManualApply={disableManualApply} applyState={applyState} onClickApply={onClickApply} onClickAccept={() => handleDiffAction("accept")} onClickReject={() => handleDiffAction("reject")}/>);
        }
    ...
    return (<TopDiv>
      <ToolbarDiv isExpanded={isExpanded} className="find-widget-skip gap-3">
        <div className="max-w-[50% flex flex-row items-center">
          <ChevronDownIcon onClick={() => setIsExpanded(!isExpanded)} className={`text-lightgray h-3.5 w-3.5 flex-shrink-0 cursor-pointer hover:brightness-125 ${isExpanded ? "rotate-0" : "-rotate-90"}`}/>
          {displayFilepath ? (<FileInfo filepath={displayFilepath} range={range} onClick={fileExists ? onClickFilename : undefined}/>) : (<span className="text-lightgray ml-2 select-none capitalize">
              {language}
            </span>)}
        </div>

        <div className="flex items-center gap-2.5">
          {!isGeneratingCodeBlock && (<div className="xs:flex hidden items-center gap-2.5">
      ...
      {isExpanded && (<div className="overflow-hidden overflow-y-auto">{children}</div>)}
    </TopDiv>);
}
