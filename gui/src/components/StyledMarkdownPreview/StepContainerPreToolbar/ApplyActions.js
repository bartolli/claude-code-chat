...
export function ApplyActions(props) {
    function onClickReject() {
        props.onClickReject();
    }
    switch (props.applyState ? props.applyState.status : null) {
        case "streaming":
            return (<div className="flex select-none items-center rounded bg-zinc-700 pl-2 pr-1">
        ...
        case "done":
            return (<div className="flex select-none items-center rounded bg-zinc-700 sm:gap-1 md:px-1.5">
          ...
          <div className="flex">
            <ToolbarButtonWithTooltip data-testid="codeblock-toolbar-reject" onClick={onClickReject} tooltipContent={`Reject all (${getMetaKeyLabel()}⇧⌫)`}>
              <XMarkIcon className="text-error h-3.5 w-3.5 hover:brightness-125"/>
            </ToolbarButtonWithTooltip>

            <ToolbarButtonWithTooltip data-testid="codeblock-toolbar-accept" onClick={props.onClickAccept} tooltipContent={`Accept all (${getMetaKeyLabel()}⇧⏎)`}>
        ...
        default:
            ...
            return (<HoverItem data-tooltip-id="codeblock-apply-code-button-tooltip" className="!p-0">
          <button data-testid="codeblock-toolbar-apply" className="text-lightgray text-[${vscForeground}] flex cursor-pointer items-center border-none bg-transparent pl-0 text-xs outline-none hover:brightness-125" onClick={props.onClickApply}>
