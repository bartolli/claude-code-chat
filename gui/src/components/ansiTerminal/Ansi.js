...
import { defaultBorderRadius, vscBackground, vscEditorBackground, vscForeground, } from "../../components";
...
function convertBundleIntoReact(linkify, useClasses, bundle, key) {
    ...
    const decorationProp = bundle.decoration
        ...
        : undefined;
    if (!linkify) {
        return (<AnsiSpan key={key} className={className || undefined} bg={useClasses ? undefined : bundle.bg} fg={useClasses ? undefined : bundle.fg} decoration={decorationProp}>
        {bundle.content}
      </AnsiSpan>);
    }
    const content = [];
    ...
    while ((match = linkRegex.exec(bundle.content)) !== null) {
        ...
        content.push(<AnsiLink key={index} href={href} target="_blank">
        {url}
      </AnsiLink>);
...
export default function Ansi(props) {
    ...
    const ansiContent = ansiToJSON(children ?? "", useClasses ?? false).map((bundle, i) => convertBundleIntoReact(linkify ?? false, useClasses ?? false, bundle, i));
    return (<StyledAnsi contentEditable="false" fontSize={getFontSize()} whiteSpace="pre-wrap" bgColor={vscBackground}>
      <pre>
