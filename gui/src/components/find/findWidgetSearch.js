export const searchWithinContainer = (containerRef, searchQuery, options) => {
    ...
    const textNodes = [];
    const walker = document.createTreeWalker(searchContainer, NodeFilter.SHOW_ALL, {
        acceptNode: (node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
                if (node.classList.contains("find-widget-skip"))
    ...
    textNodes.forEach((textNode, idx) => {
        ...
        while ((startIndex = nodeTextValue.indexOf(query, startIndex)) !== -1) {
            ...
            const left = rect.left + searchContainer.clientLeft + searchContainer.scrollLeft;
    ...
    const verticalMiddle = searchContainer.scrollTop + searchContainer.clientHeight / 2;
    let closestDist = Infinity;
    let closestMatchToMiddle = null;
...
//# sourceMappingURL=findWidgetSearch.js.map