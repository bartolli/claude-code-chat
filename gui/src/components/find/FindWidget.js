...
import { searchWithinContainer, } from "./findWidgetSearch";
import useDebounceValue from "./useDebounce";
import { useElementSize } from "./useElementSize";
const HighlightOverlay = (props) => {
    const { top, left, width, height } = props.rectangle;
    return (<div className={props.isCurrent ? "bg-findMatch-selected" : "bg-findMatch"} key={`highlight-${top}-${left}`} style={{
            position: "absolute",
            top,
            left,
            width,
...
*/
export const useFindWidget = (searchRef, headerRef, disabled) => {
    ...
    const { clientHeight: headerHeight, isResizing: headerResizing } = useElementSize(headerRef);
    ...
    const refreshSearch = useCallback((scrollTo = "none") => {
        const { results, closestToMiddle } = searchWithinContainer(searchRef, searchTerm, {
            caseSensitive,
            useRegex,
    ...
    const widget = (<div className={`fixed top-0 z-50 transition-all ${open ? "" : "-translate-y-full"} bg-vsc-background right-0 flex flex-row items-center gap-1.5 rounded-bl-lg border-0 border-b border-l border-solid border-zinc-700 pl-[3px] pr-3 sm:gap-2`}>
      <Input disabled={disabled} type="text" ref={inputRef} value={currentValue} onChange={(e) => {
      ...
      <div className="hidden flex-row gap-0.5 sm:flex">
        <HeaderButtonWithToolTip tooltipPlacement="top-end" text={"Previous Match"} onClick={(e) => {
            e.stopPropagation();
            previousMatch();
        }} className="h-4 w-4 focus:ring" disabled={matches.length < 2 || disabled}>
          <ArrowUpIcon className="h-4 w-4"/>
        </HeaderButtonWithToolTip>
        <HeaderButtonWithToolTip tooltipPlacement="top-end" text={"Next Match"} onClick={(e) => {
            e.stopPropagation();
            nextMatch();
        }} className="h-4 w-4 focus:ring" disabled={matches.length < 2 || disabled}>
      ...
      <HeaderButtonWithToolTip disabled={disabled} inverted={caseSensitive} tooltipPlacement="top-end" text={caseSensitive
            ? "Turn off case sensitivity"
            : "Turn on case sensitivity"} onClick={(e) => {
            e.stopPropagation();
      ...
      <HeaderButton inverted={false} onClick={() => setOpen(false)} className="focus:ring">
        <XMarkIcon className="h-4 w-4"/>
      </HeaderButton>
    </div>);
    // Generate the highlight overlay elements
    const highlights = useMemo(() => {
        return matches.map((match) => (<HighlightOverlay rectangle={match.overlayRectangle} isCurrent={currentMatch?.index === match.index}/>));
