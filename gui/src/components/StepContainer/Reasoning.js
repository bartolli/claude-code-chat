...
export default function Reasoning(props) {
    ...
    return (<>
      <SpoilerButton onClick={() => setOpen(!open)}>
        <ButtonContent>
          {isThinking ? (<ThinkingText>Thinking</ThinkingText>) : (`Thought for ${reasoningTime}`)}
          {open ? (<ChevronUpIcon className="h-3 w-3"/>) : (<ChevronDownIcon className="h-3 w-3"/>)}
        </ButtonContent>
      </SpoilerButton>
      {open && (<ContentDiv>
          <StyledMarkdownPreview isRenderingInStepContainer source={stripImages(props.item.reasoning.text)} itemIndex={props.index} useParentBackgroundColor/>
        </ContentDiv>)}
    </>);
}
