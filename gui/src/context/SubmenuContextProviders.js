...
import { getShortestUniqueRelativeUriPaths, getUriPathBasename, } from "core/util/uri";
...
const MAX_LENGTH = 70;
const initialContextProviders = {
    getSubmenuContextItems: () => [],
};
const SubmenuContextProvidersContext = createContext(initialContextProviders);
export const SubmenuContextProvidersProvider = ({ children, }) => {
    ...
    const disableIndexing = useAppSelector((store) => store.config.config.disableIndexing);
    ...
    useEffect(() => {
        function hasOpenFilesChanged(newFiles, oldFiles) {
        ...
        const refreshOpenFiles = async () => {
            ...
            const withUniquePaths = getShortestUniqueRelativeUriPaths(openFiles, workspaceDirs);
            ...
            if (hasOpenFilesChanged(openFileItems, lastOpenFilesRef.current)) {
                setFallbackResults((prev) => ({
                    ...prev,
                    file: deduplicateArray([...openFileItems, ...(prev.file ?? [])], (a, b) => a.id === b.id),
                }));
            }
    ...
    const providersLoading = useRef(new Set()).current;
    const abortControllers = useRef(new Map()).current;
    const getSubmenuContextItems = useCallback((providerTitle, query, limit = MAX_LENGTH) => {
        try {
            // 1. Search using minisearch
            let searchResults = [];
            if (providerTitle === undefined) {
                // Include results from all providers
                searchResults = Object.keys(minisearches).flatMap((providerTitle) => minisearches[providerTitle]
            ...
            if (searchResults.length === 0) {
                const fallbackItems = (providerTitle ? (fallbackResults[providerTitle] ?? []) : [])
                    .slice(0, limit)
                    .map((result) => {
                    return {
                        ...result,
    ...
    const loadSubmenuItems = useCallback(async (providers) => {
        await Promise.allSettled(submenuContextProviders.map(async (description) => {
            const controller = new AbortController();
            try {
                const refreshProvider = providers === "all"
                    ? true
                ...
                const deduplicatedItems = deduplicateArray(submenuItems.map((item) => ({ ...item, providerTitle })), (a, b) => a.id === b.id);
                ...
                if (providerTitle === "file") {
                    setFallbackResults((prev) => ({
                        ...prev,
                        file: deduplicateArray([...lastOpenFilesRef.current, ...itemsWithProvider], (a, b) => a.id === b.id),
                    }));
                }
                else {
                    setFallbackResults((prev) => ({
                ...
                console.error(`Error loading items for ${description.title}:`, error);
    ...
    useEffect(() => {
        ...
        const newTitles = submenuContextProviders
            .filter((provider) => !lastProviders.current.find((p) => p.title === provider.title))
    ...
    return (<SubmenuContextProvidersContext.Provider value={{
            getSubmenuContextItems,
        }}>
      {children}
    </SubmenuContextProvidersContext.Provider>);
};
export const useSubmenuContextProviders = () => useContext(SubmenuContextProvidersContext);
//# sourceMappingURL=SubmenuContextProviders.js.map