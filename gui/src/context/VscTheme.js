...
const initialVscThemeContext = {
    theme: constructTheme(window.fullColorTheme || {}),
};
export const VscThemeContext = createContext(initialVscThemeContext);
export const VscThemeProvider = ({ children, }) => {
    const [theme, setTheme] = useState(initialVscThemeContext.theme);
    useWebviewListener("setTheme", async (data) => {
        window.fullColorTheme = data.theme;
        setTheme(constructTheme(data.theme));
    });
    return (<VscThemeContext.Provider value={{ theme }}>
      {children}
    </VscThemeContext.Provider>);
};
export const useVscTheme = () => useContext(VscThemeContext);
//# sourceMappingURL=VscTheme.js.map