...
import { setLastNonEditSessionEmpty, updateEditStateApplyState, } from "../redux/slices/editState";
import { updateIndexingStatus } from "../redux/slices/indexingSlice";
import { initializeProfilePreferences, setOrganizations, setSelectedOrgId, setSelectedProfile, } from "../redux/slices/profilesSlice";
import { acceptToolCall, addContextItemsAtIndex, updateApplyState, } from "../redux/slices/sessionSlice";
...
import { setDocumentStylesFromLocalStorage, setDocumentStylesFromTheme, } from "../styles/theme";
...
function ParallelListeners() {
    ...
    const selectedProfileId = useAppSelector((store) => store.profiles.selectedProfileId);
    ...
    const handleConfigUpdate = useCallback(async (isInitial, result) => {
        const { result: configResult, profileId, organizations, selectedOrgId, } = result;
        ...
        if (isNewProfileId) {
            dispatch(initializeProfilePreferences({
    ...
    const initialLoadAuthAndConfig = useCallback(async (initial) => {
        const result = await ideMessenger.request("config/getSerializedProfileInfo", undefined);
    ...
    useWebviewListener("configUpdate", async (update) => {
        if (!update) {
            return;
        }
    ...
    useWebviewListener("jetbrains/setColors", async (data) => {
    ...
    useWebviewListener("getCurrentSessionId", async () => {
        return sessionId;
    }, [sessionId]);
    ...
    useWebviewListener("updateApplyState", async (state) => {
        if (state.streamId === EDIT_MODE_STREAM_ID) {
            ...
            if (state.status === "closed" &&
                ...
                void dispatch(streamResponseAfterToolCall({
                    toolCallId: currentToolCallApplyState.toolCallId,
                }));
            }
        }
...
//# sourceMappingURL=ParallelListeners.js.map