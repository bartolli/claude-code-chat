...
import { clearThemeLocalCache, setDocumentStylesFromTheme, THEME_COLORS, varWithFallback, } from "./theme";
const ThemeTailwindClassExample = ({ colorName, varNames, defaultColor, isMissing, }) => {
    return (<>
      <div className={`line-clamp-1 break-all px-2 text-right`}>
        {colorName}
      </div>
      <div className={`line-clamp-1 break-all p-1 text-[9px] ${isMissing ? "text-error" : ""}`} style={{
            backgroundColor: varWithFallback(colorName),
        }}>
        {varNames.join(", ")}
      </div>
      <div className={`line-clamp-1 break-all p-1 text-[9px]`} style={{
...
function ThemePage() {
    ...
    return (<div className="flex flex-1 flex-col gap-3 px-2 py-2">
      <span className="flex cursor-pointer flex-row items-center gap-2" onClick={() => navigate(ROUTES.HOME)}>
        <span className="">←</span>
        Back to Chat
      </span>
      ...
      <div className="bg-background border-border grid grid-cols-1 gap-4 rounded-md border border-solid p-4 md:grid-cols-2">
        ...
        <input className="bg-input text-input-foreground border-input-border placeholder:text-input-placeholder focus:border-border-focus rounded-md border border-solid p-2 focus:outline-none" placeholder="Input example"/>
        ...
        <div onClick={() => setListToggled(!listToggled)} className={`cursor-pointer p-2 ${listToggled ? "bg-list-active text-list-active-foreground" : "bg-background text-foreground hover:bg-list-hover"}`}>
          List item (clickable)
        </div>
      ...
      {missingVars.length > 0 ? (<div className="flex flex-col gap-1">
          {missingVars.map((varName) => (<div key={varName} className="">
              {varName}
            </div>))}
        </div>) : (<div className="">No missing variables</div>)}
      <Button onClick={refreshColors}>Refresh Missing Colors</Button>
      {jetbrains ? (<Button onClick={clearThemeLocalCache}>Clear Theme Cache</Button>) : null}
      ...
      <div className="grid grid-cols-3">
        ...
        {Object.entries(THEME_COLORS).map(([colorName, val]) => (<ThemeTailwindClassExample isMissing={missingVars.includes(colorName)} key={colorName} colorName={colorName} defaultColor={val.default} varNames={val.vars}/>))}
      </div>
    </div>);
}
