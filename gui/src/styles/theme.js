...
export const THEME_CSS_VARS = Object.values(THEME_COLORS)
    ...
    .flat();
export const THEME_CSS_VAR_DEFAULTS = Object.entries(THEME_COLORS).reduce((acc, [_, value]) => {
    value.vars.forEach((varName) => {
        acc[varName] = value.default;
    });
    return acc;
}, {});
export const THEME_DEFAULTS = Object.entries(THEME_COLORS).reduce((acc, [key, value]) => {
...
export const getRecursiveVar = (vars, defaultColor) => {
    return [...vars].reverse().reduce((curr, varName) => {
        return `var(${varName}, ${curr})`;
    }, defaultColor);
};
export const varWithFallback = (colorName) => {
...
export const setDocumentStylesFromTheme = (theme) => {
    // Check for extraneous theme items
    Object.entries(theme).forEach(([colorName, value]) => {
        const themeVals = THEME_COLORS[colorName];
        if (!themeVals) {
            console.warn(`Receieved theme color ${colorName} which is not used by the theme`);
            return;
        }
    });
...
export const setDocumentStylesFromLocalStorage = (checkCache) => {
