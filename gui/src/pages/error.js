...
const ErrorPage = () => {
    ...
    return (<div className="flex flex-col items-center justify-center px-2 py-4 text-center sm:px-8">
      ...
      <Button className="flex flex-row items-center gap-2" onClick={() => {
        ...
        {initialLoad ? (<FlagIcon className="h-5 w-5 text-red-600"/>) : (<ArrowPathIcon className="h-5 w-5"/>)}
        Continue
      </Button>
      ...
      <div className="mt-2 flex flex-row flex-wrap justify-center gap-2">
        <SecondaryButton onClick={() => openUrl(GITHUB_LINK)} className="flex items-center justify-center space-x-2 rounded-lg px-4 py-2 text-base text-white">
          <GithubIcon size={20}/> <span className="ml-2">GitHub</span>
        </SecondaryButton>
        <SecondaryButton onClick={() => openUrl(DISCORD_LINK)} className="flex items-center justify-center rounded-lg text-base">
