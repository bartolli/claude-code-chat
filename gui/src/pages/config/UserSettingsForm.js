import { CheckIcon, ChevronRightIcon, ExclamationTriangleIcon, XMarkIcon, } from "@heroicons/react/24/outline";
...
export function UserSettingsForm() {
    ...
    function handleUpdate(sharedConfig) {
        // Optimistic update
    ...
    const disableAutocompleteInFiles = (config.tabAutocompleteOptions?.disableInFiles ?? []).join(", ");
    const [formDisableAutocomplete, setFormDisableAutocomplete] = useState(disableAutocompleteInFiles);
    useEffect(() => {
    ...
    const useCurrentFileAsContext = config.experimental?.useCurrentFileAsContext ?? false;
    ...
    const useAutocompleteMultilineCompletions = config.tabAutocompleteOptions?.multilineCompletions ?? "auto";
    ...
    return (<div className="flex flex-col">
      ...
      {hubEnabled ? (<div className="flex flex-col gap-4 py-4">
          <div>
            <h2 className="mb-2 mt-0 p-0">User settings</h2>
          </div>

          <div className="flex flex-col gap-4">
            <ToggleSwitch isToggled={showSessionTabs} onToggle={() => handleUpdate({
                showSessionTabs: !showSessionTabs,
            })} text="Show Session Tabs"/>
            <ToggleSwitch isToggled={codeWrap} onToggle={() => handleUpdate({
                codeWrap: !codeWrap,
            })} text="Wrap Codeblocks"/>

            <ToggleSwitch isToggled={showChatScrollbar} onToggle={() => handleUpdate({
                showChatScrollbar: !showChatScrollbar,
            })} text="Show Chat Scrollbar"/>
            <ToggleSwitch isToggled={readResponseTTS} onToggle={() => handleUpdate({
            ...
            {/* <ToggleSwitch
                  ...
                  /> */}
            <ToggleSwitch isToggled={!disableSessionTitles} onToggle={() => handleUpdate({
                disableSessionTitles: !disableSessionTitles,
            })} text="Enable Session Titles"/>
            <ToggleSwitch isToggled={!displayRawMarkdown} onToggle={() => handleUpdate({
                displayRawMarkdown: !displayRawMarkdown,
            })} text="Format Markdown"/>

            <ToggleSwitch isToggled={allowAnonymousTelemetry} onToggle={() => handleUpdate({
                allowAnonymousTelemetry: !allowAnonymousTelemetry,
            })} text="Allow Anonymous Telemetry"/>

            <ToggleSwitch isToggled={!disableIndexing} onToggle={() => handleUpdate({
            ...
            <label className="flex items-center justify-between gap-3">
              ...
              <Select value={useAutocompleteMultilineCompletions} onChange={(e) => handleUpdate({
                useAutocompleteMultilineCompletions: e.target.value,
            })}>
            ...
            <label className="flex items-center justify-between gap-3">
              ...
              <NumberInput value={modelTimeout} onChange={(val) => handleUpdate({
                modelTimeout: val,
            })} min={100} max={5000}/>
            </label>
            <label className="flex items-center justify-between gap-3">
              <span className="text-left">Autocomplete Debounce (ms)</span>
              <NumberInput value={debounceDelay} onChange={(val) => handleUpdate({
                debounceDelay: val,
            })} min={0} max={2500}/>
            </label>
            <form className="flex flex-col gap-1" onSubmit={(e) => {
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Input value={formDisableAutocomplete} className="max-w-[100px]" onChange={(e) => {
                setFormDisableAutocomplete(e.target.value);
            }}/>
                  <div className="flex h-full flex-col">
                    {formDisableAutocomplete !== disableAutocompleteInFiles ? (<>
                        <div onClick={handleDisableAutocompleteSubmit} className="cursor-pointer">
                          <CheckIcon className="h-4 w-4 text-green-500 hover:opacity-80"/>
                        </div>
                        <div onClick={cancelChangeDisableAutocomplete} className="cursor-pointer">
          ...
          <div className="flex flex-col gap-x-2 gap-y-4">
            <div className="flex cursor-pointer items-center gap-2 text-left text-sm font-semibold" onClick={() => setShowExperimental(!showExperimental)}>
              <ChevronRightIcon className={`h-4 w-4 transition-transform ${showExperimental ? "rotate-90" : ""}`}/>
              <span>Experimental Settings</span>
            </div>
            <div className={`duration-400 overflow-hidden transition-all ease-in-out ${showExperimental ? "max-h-40" : "max-h-0"}`}>
              <div className="flex flex-col gap-x-1 gap-y-4 pl-6">
                <ToggleSwitch isToggled={autoAcceptEditToolDiffs} onToggle={() => handleUpdate({
                autoAcceptEditToolDiffs: !autoAcceptEditToolDiffs,
            })} text="Auto-Accept Agent Edits" showIfToggled={<>
                      <ExclamationTriangleIcon data-tooltip-id={`auto-accept-diffs-warning-tooltip`} className="h-3 w-3 text-yellow-500"/>
                ...
                <ToggleSwitch isToggled={useCurrentFileAsContext} onToggle={() => handleUpdate({
...
//# sourceMappingURL=UserSettingsForm.js.map