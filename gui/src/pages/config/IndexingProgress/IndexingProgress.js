...
function IndexingProgress() {
    ...
    function onClickRetry() {
        ...
        if (update.shouldClearIndexes && !isJetBrains()) {
            ...
            dispatch(setDialogMessage(<ConfirmationDialog title="Rebuild codebase index" confirmText="Rebuild" text={"Your index appears corrupted. We recommend clearing and rebuilding it, " +
                    "which may take time for large codebases.\n\n" +
                    "For a faster rebuild without clearing data, press 'Shift + Command + P' to open " +
                    "the Command Palette, and type out 'Continue: Force Codebase Re-Indexing'"} onConfirm={() => {
    ...
    return (<div className="mt-4 flex flex-col">
      <div className="mb-0 flex justify-between text-sm">
        ...
        {update.status !== "loading" && (<IndexingProgressIndicator update={update}/>)}
      </div>
      ...
      {update.status === "failed" && (<div className="mt-4">
