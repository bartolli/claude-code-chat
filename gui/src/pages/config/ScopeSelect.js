import { BuildingOfficeIcon, ChevronUpDownIcon, } from "@heroicons/react/24/outline";
...
import { selectCurrentOrg, setSelectedOrgId, } from "../../redux/slices/profilesSlice";
export function ScopeSelect({ onSelect }) {
    const { organizations } = useAuth();
    const ideMessenger = useContext(IdeMessengerContext);
    const selectedOrgId = useAppSelector((state) => state.profiles.selectedOrganizationId);
      ...
      <div className="relative">
        <ListboxButton className="hover:bg-list-active hover:text-list-active-foreground w-full min-w-[140px] justify-between px-4 py-2 sm:min-w-[200px]">
          <div className="flex items-center gap-2">
            {selectedDisplay?.iconUrl ? (<img src={selectedDisplay.iconUrl} alt="" className="h-5 w-5"/>) : (<CurScopeEntityFallBackIcon className="h-5 w-5"/>)}
        ...
        <ListboxOptions className="z-[1000] min-w-[140px] pt-0.5 sm:min-w-[200px]">
          {organizations.map((org) => (<ListboxOption key={org.id} value={org.id} className="py-2">
              <div className="flex items-center gap-2">
                {org.iconUrl ? (<img src={org.iconUrl} alt="" className="h-5 w-5"/>) : (<BuildingOfficeIcon className="h-5 w-5"/>)}
                <span>{org.name}</span>
              </div>
            </ListboxOption>))}
        </ListboxOptions>
