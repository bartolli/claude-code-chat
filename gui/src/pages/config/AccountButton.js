...
import { Button, Popover, PopoverButton, PopoverPanel, Transition, } from "../../components/ui";
...
export function AccountButton() {
    ...
    if (!session) {
        return (<Button variant="outline" className="mb-1 whitespace-nowrap py-1" onClick={() => login(false)}>
        Sign in
      </Button>);
    }
          ...
          <Transition>
            <PopoverPanel className="bg-vsc-input-background xs:p-4 absolute right-0 mt-1 rounded-md border border-zinc-700 p-2 shadow-lg">
              <div className="flex flex-col gap-3">
                ...
                {organizations.length > 0 && (<div className="flex flex-col gap-1">
                ...
                <Button variant="ghost" onClick={logout} className="!mx-0 w-full">
                  Sign out
                </Button>
              </div>
            </PopoverPanel>
