...
export const ArgsToggleIcon = ({ isShowing, setIsShowing, toolCallId, }) => {
    const argsTooltipId = useMemo(() => {
        return "args-hover-" + toolCallId;
    }, [toolCallId]);
    return (<>
      <div data-tooltip-id={argsTooltipId} onClick={(e) => {
            e.stopPropagation();
...
export const ArgsItems = ({ args, isShowing }) => {
    if (args.length === 0) {
        return null;
    }
    if (!isShowing) {
    ...
    return (<div className="ml-5 mr-2 mt-1 flex flex-col text-xs">
      {args.map(([key, value]) => (<div key={key} className="flex flex-row items-center gap-2 py-0.5">
...
//# sourceMappingURL=ToolCallArgs.js.map