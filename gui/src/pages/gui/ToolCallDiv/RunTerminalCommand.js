...
const StatusIcon = styled.span `
  ...
  border-radius: 50%;
  margin-right: 8px;
  background-color: ${(props) => props.status === "running"
...
const TerminalContainer = styled.div `
  margin-bottom: 16px;
`;
export function RunTerminalCommand(props) {
    const dispatch = useAppDispatch();
    // Find the terminal output from context items if available
    const terminalItem = props.toolCallState.output?.find((item) => item.name === "Terminal");
    ...
    return (<TerminalContainer>
      {/* Command */}
      <StyledMarkdownPreview isRenderingInStepContainer source={`\`\`\`bash .sh\n$ ${props.command ?? ""}\n\`\`\``}/>

      {/* Terminal output with ANSI support */}
      {isRunning && !hasOutput && (<WaitingMessage>Waiting for output...</WaitingMessage>)}
      {hasOutput && (<AnsiWrapper>
          <Ansi>{terminalContent}</Ansi>
        </AnsiWrapper>)}

      {/* Status information */}
      {(statusMessage || isRunning) && (<CommandStatus>
          <StatusIcon status={statusType}/>
          {isRunning ? "Running" : statusMessage}
          {isRunning && props.toolCallId && (<BackgroundLink onClick={() => {
