import { ArrowRightIcon, CheckIcon, CodeBracketIcon, DocumentIcon, DocumentTextIcon, FolderIcon, FolderOpenIcon, GlobeAltIcon, MagnifyingGlassIcon, MapIcon, PencilIcon, XMarkIcon, } from "@heroicons/react/24/outline";
...
export function ToolCallDiv(props) {
    const availableTools = useAppSelector((state) => state.config.config.tools);
    const tool = useMemo(() => {
        return availableTools.find((tool) => props.toolCall.function?.name === tool.function.name);
    }, [availableTools, props.toolCall]);
    const icon = props.toolCall.function?.name &&
        toolCallIcons[props.toolCall.function.name];
    if (icon) {
        return (<SimpleToolCallUI tool={tool} toolCallState={props.toolCallState} icon={props.toolCallState.status === "generated" ? ArrowRightIcon : icon} contextItems={props.output ?? []}/>);
    }
    return (<ToolCallDisplay icon={getStatusIcon(props.toolCallState.status)} tool={tool} toolCallState={props.toolCallState}>
      <FunctionSpecificToolCallDiv toolCall={props.toolCall} toolCallState={props.toolCallState} historyIndex={props.historyIndex}/>
    </ToolCallDisplay>);
}
