...
import { getLocalStorage, LocalStorageKey, setLocalStorage, } from "../../util/localStorage";
const useTutorialListener = (onTutorialClosed) => {
    useWebviewListener("didCloseFiles", async (data) => {
        const uris = data?.uris ?? [];
        const isTutorial = uris.some((uri) => {
            const lowercaseUri = uri.toLowerCase();
            return (lowercaseUri.endsWith("continue_tutorial.py") ||
...
//# sourceMappingURL=ExploreDialogWatcher.js.map