import { ArrowPathIcon, ArrowTopRightOnSquareIcon, ClipboardIcon, Cog6ToothIcon, KeyIcon, } from "@heroicons/react/24/outline";
...
function parseErrorMessage(fullErrMsg) {
...
const StreamErrorDialog = ({ error }) => {
    ...
    const parsedError = useMemo(() => parseErrorMessage(error?.message || ""), [error]);
    ...
    if (selectedModel) {
        ...
        const foundProvider = Object.values(providers).find((p) => p?.provider === selectedModel.provider);
        if (foundProvider) {
    ...
    const checkKeysButton = apiKeyUrl ? (<GhostButton className="flex items-center" onClick={() => {
            ideMessenger.post("openUrl", apiKeyUrl);
        }}>
      <KeyIcon className="mr-1.5 h-3.5 w-3.5"/>
      <span>View key</span>
    </GhostButton>) : null;
    const configButton = (<GhostButton className="flex items-center" onClick={() => {
    ...
    const resubmitButton = (<GhostButton className="flex items-center" onClick={() => {
            let index = -1;
            ...
            const editorState = index === -1 ? mainEditor.getJSON() : history[index].editorState;
    ...
    if (parsedError === "You have exceeded the chat limit for the Models Add-On.") {
        return <ModelsAddOnLimitDialog />;
    }
    let errorContent = (<div className="mb-1 mt-3">
    ...
    if (statusCode === 429) {
        errorContent = (<div className="flex flex-col gap-2">
        <span>
    ...
    if (statusCode === 401) {
        errorContent = (<div className="flex flex-col gap-2">
        {session && selectedProfile && !isLocalProfile(selectedProfile) && (<div className="flex flex-col gap-1">
    ...
    console.log({ message });
    return (<div className="flex flex-col gap-4 px-3 pb-3 pt-3">
      {/* Concise error title */}
          ...
          <ToggleDiv title="View error output" testId="error-output-toggle">
            <div className="flex flex-col gap-0 rounded-sm">
              ...
              <div className="flex flex-row items-center justify-end gap-2 p-2">
                <GhostButton onClick={copyErrorToClipboard} className="flex items-center">
      ...
      <div>
        ...
        <div className="mt-2 flex flex-row flex-wrap items-center gap-2">
          <GhostButton className="flex flex-row items-center gap-2 rounded px-3 py-1.5" onClick={() => {
