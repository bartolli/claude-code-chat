...
suite('Message Flow Integration Tests - Baseline', () => {
  ...
  /**
   * Array to capture all messages sent through the webview protocol during tests
   */
  const capturedMessages: Array<{
    /** Message type identifier for routing in the webview protocol */
    type: string;
    /** Message payload data containing the actual content */
    data: any;
  }> = [];

  /**
   * Mock implementation of Claude service for testing message flow without actual API calls
   */
  class MockClaudeService implements Partial<ClaudeServiceInterface> {
    /**
     * Simulates sending a message to Claude and receiving a response
     * @param text - The message text to send
     * @param options - Optional message sending options
     * @returns Mock response object with message metadata
     */
    ...
    /**
     * Simulates streaming message chunks from Claude API
     * @param text - The message text to send
     * @param options - Options including onChunk callback for handling stream chunks
     */
  ...
  test('User message flow - complete lifecycle', async () => {
    ...
    assert.strictEqual(
      messageAddEvents.length >= 2,
      true,
      'Should have at least 2 message/add events (user + assistant)'
    );
  ...
  test('Token counting and cost tracking', async () => {
    ...
    const tokenMessages = capturedMessages.filter(
      (m) => m.type === 'message/tokensUpdate' || m.type === 'session/tokensUpdated'
    );
  ...
  test('Error handling and recovery', async () => {
    ...
    const errorMessages = capturedMessages.filter(
      (m) => m.type === 'message/error' || m.type === 'ui/showError'
    );
...
export const WEBVIEW_MESSAGE_TYPES = {
  ...
  'session/messageAppended': {
    description: 'Content appended to message',
    payload: { messageId: 'string', content: 'string' },
  },
  'session/tokensUpdated': {
    description: 'Token count updated',
  ...
  'message/thinking': {
    description: 'Thinking block update',
    payload: { content: 'string', isActive: 'boolean', messageId: 'string' },
  },
  'message/toolUse': {
    description: 'Tool use event',
    payload: { id: 'string', name: 'string', input: 'object' },
  },
  'message/toolResult': {
    description: 'Tool result',
    payload: { toolUseId: 'string', result: 'any' },
  },
  'message/tokensUpdate': {
    description: 'Token usage update',
    payload: { input: 'number', output: 'number' },
  },
