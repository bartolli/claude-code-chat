...
export class MigrationTestHarness {
  ...
  /** Action mapper for testing */
  private actionMapper: ActionMapper;
  /** State comparator for validation */
  ...
  private async testActionMapping(): Promise<TestResult> {
    ...
    try {
      ...
      for (const action of testActions) {
        ...
        results.push(
          `${status} ${action.type}: ${result.success ? 'mapped' : result.error || 'unmapped'}`
        );
  ...
  private async testStateComparison(): Promise<TestResult> {
    ...
    const simpleStateManager =
      this.context.workspaceState.get<SimpleStateManager>('simpleStateManager');
    ...
    // Create comparator
    this.stateComparator = new StateComparator(simpleStateManager, reduxStateManager, this.context);
    ...
    try {
      ...
      const criticalActions = [
        ...
        { type: 'ui/setReady', payload: { ready: true } },
        {
          ...
          payload: { toolName: 'cmd', toolId: 't1', toolInput: {} },
        },
      ...
      for (const action of criticalActions) {
        ...
        if (result.success) {
          ...
          this.outputChannel.appendLine(
            `  ✅ ${action.type}: mapped to ${result.mappedAction?.type}`
          );
      ...
      this.outputChannel.appendLine(
        `  Need custom handler: ${results.customHandler.length} actions`
      );
...
export interface TestResult {
  ...
  /** Test result message */
  message: string;
  /** Optional additional test details */
