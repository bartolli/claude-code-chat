...
export class WebviewProtocol {
    ...
    private streamHandlers2: Record<string, (data: any) => AsyncGenerator<any, void, unknown>> = {};
    ...
    on<T extends FromWebviewMessageType>(
        messageType: T,
        handler: (data: FromWebviewProtocol[T][0]) => Promise<FromWebviewProtocol[T][1]> | FromWebviewProtocol[T][1]
    ): void {
    ...
    onStream<T extends FromWebviewMessageType>(
        messageType: T,
        handler: (data: FromWebviewProtocol[T][0]) => AsyncGenerator<FromWebviewProtocol[T][1], void, unknown>
    ): void {
        this.streamHandlers2[messageType] = handler as any;
    }
    ...
    private async handleMessage(message: Message): Promise<void> {
        ...
        if (this.streamHandlers2[messageType as FromWebviewMessageType]) {
        ...
        if (this.handlers[messageType as FromWebviewMessageType]) {
        ...
        this.respondError(messageId, messageType, new Error(`No handler for message type: ${messageType}`));
    }
    ...
    postLegacy(type: string, data: any): void {
        ...
        if (newType) {
            this.post(newType as ToWebviewMessageType, data);
        } else {
