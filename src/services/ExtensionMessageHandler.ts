...
export class ExtensionMessageHandler {
  ...
  /**
   * Attaches the webview protocol for communication
   * @param webviewProtocol - Protocol instance for webview communication
   */
  ...
  /**
   * Handles chat messages sent to Claude
   * @param data - Chat message data with text and optional mode flags
   * @param data.text - The message text content to send to Claude
   * @param data.planMode - Whether to enable plan mode for this message
   * @param data.thinkingMode - Whether to enable thinking mode for this message
   */
  private async handleChatMessage(data: {
    /**
     * The message text content to send to Claude
     */
    text: string;
    /**
     * Whether to enable plan mode for this message
     */
    planMode?: boolean;
    /**
     * Whether to enable thinking mode for this message
     */
    ...
    try {
      ...
      if (data.planMode) {
        this.outputChannel.appendLine(
          '[DEBUG] Plan mode requested - needs implementation in ClaudeProcessManager'
        );
      }
      ...
      if (!hasApiKey) {
        this.logger.info(
          ...
          'No ANTHROPIC_API_KEY found, will use interactive Claude CLI (requires Pro/Team subscription)'
        );
      ...
      try {
        ...
        if (mcpConfig) {
          this.outputChannel.appendLine(
            `[MCP] Found .mcp.json with ${Object.keys(mcpConfig.mcpServers).length} servers:`
          );
          Object.entries(mcpConfig.mcpServers).forEach(([name, config]) => {
            this.outputChannel.appendLine(
              `[MCP]   - ${name}: ${config.command} ${config.args?.join(' ') || ''}`
            );
          });
        } else {
        ...
        this.outputChannel.appendLine(
          `[MCP]   Local: ${allConfigs.local ? Object.keys(allConfigs.local.mcpServers).length + ' servers' : 'none'}`
        );
        this.outputChannel.appendLine(
          `[MCP]   Project: ${allConfigs.project ? Object.keys(allConfigs.project.mcpServers).length + ' servers' : 'none'}`
        );
        this.outputChannel.appendLine(
          `[MCP]   User: ${allConfigs.user ? Object.keys(allConfigs.user.mcpServers).length + ' servers' : 'none'}`
        );

        // Test merged configuration
        const mergedServers = mcpService.mergeConfigs(
          allConfigs.local,
          allConfigs.project,
          allConfigs.user
        );
        ...
        mergedServers.forEach((server) => {
          this.outputChannel.appendLine(
            `[MCP]   - ${server.name} (${server.scope}): ${server.command}`
          );
        });
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            this.outputChannel.appendLine(
              `[DEBUG] Fallback output: ${fallbackResult.substring(0, 200)}...`
            );
      ...
      this.outputChannel.appendLine(
        `[DEBUG] ANTHROPIC_API_KEY: ${processEnv.ANTHROPIC_API_KEY ? 'Set' : 'Not set'}`
      );
      ...
      const timeout = setTimeout(() => {
        ...
        this.outputChannel.appendLine(
          `[WARNING] Process timeout after ${PROCESS_TIMEOUT_MS / 1000} seconds (inactive for ${inactiveTime / 1000}s)`
        );
      ...
      claudeProcess.stdout?.on('readable', () => {
        ...
        if (data) {
          this.outputChannel.appendLine(
            `[DEBUG] Readable event - data available: ${data.length} bytes`
          );
        }
      });
      ...
      setTimeout(() => {
        if (
          !receivedData &&
          this.currentClaudeProcess &&
          this.currentClaudeProcess.exitCode === null
        ) {
          this.outputChannel.appendLine(`[WARNING] No data received after 2 seconds`);
          this.outputChannel.appendLine(
            `[WARNING] Process still running: ${!this.currentClaudeProcess.killed}`
          );
          this.outputChannel.appendLine(
            `[WARNING] Exit code: ${this.currentClaudeProcess.exitCode}`
          );
          ...
          if (claudeProcess.stdout && claudeProcess.stdout instanceof Readable) {
            this.outputChannel.appendLine(
              `[WARNING] Stdout readable: ${claudeProcess.stdout.readable}`
            );
            this.outputChannel.appendLine(
              `[WARNING] Stdout destroyed: ${claudeProcess.stdout.destroyed}`
            );
          }
        }
      }, 2000);
          ...
          for (const line of lines) {
            if (line.trim()) {
              try {
                ...
                this.processClaudeStreamMessage(
                  json,
                  (content) => {
                  ...
                  (metadata) => {
                    if (metadata.sessionId) {
                      processSessionId = metadata.sessionId;
                    }
                    ...
                    if (metadata.apiKeySource) {
                      apiKeySource = metadata.apiKeySource;
                    }
                  },
                  streamingState
                );
        ...
        claudeProcess.stdout.on('end', () => {
          ...
          if (jsonBuffer.trim()) {
            try {
              ...
              this.outputChannel.appendLine(
                `[DEBUG] Failed to parse remaining buffer: ${jsonBuffer}`
              );
            }
          }
          ...
          this.outputChannel.appendLine(
            `[DEBUG] Stream ended - first text block updated thinking, subsequent blocks created new messages`
          );
        });
      ...
      claudeProcess.on('exit', (code, signal) => {
        ...
        if (this.pendingToolIds.size > 0) {
          this.outputChannel.appendLine(
            `[Process Exit] ${this.pendingToolIds.size} tools did not receive results`
          );
          this.outputChannel.appendLine(
            `[Process Exit] Pending tool IDs: ${Array.from(this.pendingToolIds).join(', ')}`
          );
  ...
  /**
   * Processes Claude stream messages in JSON format
   * @param json - The JSON message from Claude stream
   * @param onContent - Callback for content updates
   * @param onMetadata - Callback for metadata updates
   * @param streamingState - State tracking for streaming status
   * @param streamingState.isStreaming - Whether the stream is currently active
   */
    ...
    switch (json.type) {
      case 'system':
        if (json.subtype === 'init') {
          this.outputChannel.appendLine(
            `[JSON] System init - Model: ${json.model}, Session: ${json.session_id}`
          );
          ...
          if (json.mcp_servers && Array.isArray(json.mcp_servers)) {
            this.outputChannel.appendLine(
              `[JSON] MCP Servers: ${json.mcp_servers.length} servers found`
            );
            ...
            const serversWithCounts = json.mcp_servers.map((server: any) => {
              ...
              if (json.tools && Array.isArray(json.tools)) {
                ...
                this.outputChannel.appendLine(
                  `[JSON]   - Tool count for ${server.name}: ${toolCount}`
                );
              }
          ...
          if (!streamingState.isStreaming) {
            ...
            this.outputChannel.appendLine(
              `[DEBUG] Streaming started - waiting for content blocks to create messages`
            );
          }
        }
        break;
      ...
      case 'assistant':
        ...
        this.outputChannel.appendLine(
          `[DEBUG] Message structure: ${JSON.stringify(json.message, null, 2).substring(0, 500)}...`
        );
          ...
          if (Array.isArray(json.message.content)) {
            ...
            for (const block of json.message.content) {
              if (block.type === 'text' && block.text) {
                this.outputChannel.appendLine(
                  `[JSON] Text content: ${block.text.substring(0, 50)}...`
                );

                // Update messageId if we have one but the message doesn't
                if (
                  this.hasCreatedAssistantMessage &&
                ...
                if (trimmedText.startsWith('{') && trimmedText.endsWith('}')) {
                  try {
                    ...
                    if (planData.type === 'plan') {
                      this.outputChannel.appendLine(
                        `[JSON] Detected plan response (entire message)`
                      );
                ...
                if (jsonMatch) {
                  try {
                    ...
                    if (planData.type === 'plan') {
                      this.outputChannel.appendLine(
                        `[JSON] Detected plan response (extracted from text)`
                      );
                      ...
                      const textBeforeJson = trimmedText.substring(
                        0,
                        trimmedText.indexOf(jsonMatch[0])
                      );
                      const textAfterJson = trimmedText.substring(
                        trimmedText.indexOf(jsonMatch[0]) + jsonMatch[0].length
                      );
                ...
                if (this.webviewProtocol) {
                  ...
                  this.outputChannel.appendLine(
                    `[DEBUG] Created new assistant message: ${newMessageId}`
                  );
                ...
                this.outputChannel.appendLine(
                  `[JSON] Thinking content: ${block.thinking.substring(0, 50)}...`
                );
                ...
                if (!this.hasCreatedAssistantMessage || !this.currentAssistantMessageId) {
                  ...
                  if (this.webviewProtocol) {
                    ...
                    this.hasCreatedAssistantMessage = true;
                    this.outputChannel.appendLine(
                      `[DEBUG] Created assistant message with thinking: ${messageId}`
                    );
                  }
                } else {
                ...
                if (!this.thinkingStartTime) {
                  this.thinkingStartTime = Date.now();
                  this.outputChannel.appendLine(
                    `[DEBUG] Thinking started at: ${this.thinkingStartTime}`
                  );
                }
                ...
                this.outputChannel.appendLine(
                  `[DEBUG] webviewProtocol status: ${this.webviewProtocol ? 'available' : 'not available'}`
                );
                if (this.webviewProtocol) {
                  this.outputChannel.appendLine(
                  ...
                  this.outputChannel.appendLine(
                    `[ERROR] Cannot send thinking update - webviewProtocol not available`
                  );
                }
              } else if (block.type === 'tool_use') {
                this.outputChannel.appendLine(
                  `[JSON] Tool use: ${block.name} with id: ${block.id}`
                );
                this.outputChannel.appendLine(
                  `[JSON] Tool input: ${JSON.stringify(block.input, null, 2)}`
                );
            ...
            this.outputChannel.appendLine(
              `[JSON] String content: ${json.message.content.substring(0, 50)}...`
            );
        ...
        if (json.message?.usage) {
          this.outputChannel.appendLine(
            `[JSON] Token usage - input: ${json.message.usage.input_tokens}, output: ${json.message.usage.output_tokens}, thinking: ${json.message.usage.thinking_tokens || 0}`
          );
          ...
          this.webviewProtocol?.post('message/tokenUsage', {
            ...
            cacheTokens:
              (json.message.usage.cache_creation_input_tokens || 0) +
        ...
        if (json.message?.content && Array.isArray(json.message.content)) {
          for (const block of json.message.content) {
            if (block.type === 'tool_result' && block.tool_use_id) {
              ...
              this.outputChannel.appendLine(
                `[JSON] Tool result block structure: ${JSON.stringify(block, null, 2)}`
              );
              ...
              if (!resultText && block.output) {
                ...
                this.outputChannel.appendLine(
                  `[JSON] Found result in 'output' field: ${resultText?.substring(0, 100)}...`
                );
                ...
                this.outputChannel.appendLine(
                  `[JSON] Extracted MCP result text: ${resultText?.substring(0, 100)}...`
                );
                ...
                this.outputChannel.appendLine(
                  `[JSON] Found result in 'content' field: ${resultText?.substring(0, 100)}...`
                );
              }
              ...
              if (!resultText) {
                this.outputChannel.appendLine(
                  `[JSON] WARNING: No result text found for tool ${block.tool_use_id}`
                );
      ...
      case 'result':
        this.outputChannel.appendLine(
          `[JSON] Result - Success: ${!json.is_error}, Cost: $${json.total_cost_usd}`
        );
      ...
      default:
        ...
        this.logger.debug('ExtensionMessageHandler', 'Unknown message type', {
          type: json.type,
          json,
        });
    }
   ...
   * @returns True if the tool requires permission
   */
  private toolNeedsPermission(toolName: string): boolean {
    // Tools that typically require permission
    const permissionRequiredTools = ['Bash', 'Write', 'Edit', 'MultiEdit', 'git', 'npm', 'yarn'];
   ...
   * @param data - Permission response data containing approval status
   * @param data.toolId - Unique identifier for the tool request
   * @param data.approved - Whether the tool use was approved by the user
   * @param data.toolName - Name of the tool being approved or rejected
   */
  private handlePermissionResponse(data: {
    ...
    toolId: string;
    /**
     * Whether the tool use was approved by the user
     */
    approved: boolean;
    /**
     * Name of the tool being approved or rejected
     */
    toolName: string;
  }): void {
    this.outputChannel.appendLine(
      `[Permission] User ${data.approved ? 'APPROVED' : 'DENIED'} ${data.toolName} (${data.toolId})`
    );
   ...
   * @param stream - Readable stream from Claude process
   * @param onContent - Callback for content updates
   * @param onMetadata - Callback for metadata updates
  ...
  private async processStreamWithStreamProcessor(
    ...
    try {
      ...
      for await (const chunk of chunks) {
        ...
        this.outputChannel.appendLine(
          `[StreamProcessor] Raw JSON: ${JSON.stringify(json, null, 2)}`
        );
        ...
        switch (json.type) {
          ...
          case 'assistant':
            ...
            if (json.message?.content) {
              if (Array.isArray(json.message.content)) {
                for (const block of json.message.content) {
                  ...
                  if (block.type === 'text' && block.text) {
                    this.outputChannel.appendLine(
                      `  - Text content: ${block.text.substring(0, 100)}...`
                    );
                ...
                this.outputChannel.appendLine(
                  `  - String content: ${json.message.content.substring(0, 100)}...`
                );
            ...
            if (json.message?.usage) {
              this.outputChannel.appendLine(
                `  - Token usage: input=${json.message.usage.input_tokens}, output=${json.message.usage.output_tokens}`
              );
            }
          ...
          default:
            this.outputChannel.appendLine(
              `[StreamProcessor] Unknown message type: ${(json as ClaudeStreamMessage).type || 'undefined'}`
            );
        }
      }

      this.outputChannel.appendLine(
        `[StreamProcessor] Stream processing completed. Total chunks: ${chunkCount}`
      );
    } catch (error) {
   ...
   * @returns Promise that resolves when command is launched
   */
  ...
  /**
   * Loads MCP server configurations and sends them to the UI
   * @returns Promise that resolves when servers are loaded and sent
   */
  private async loadAndSendMcpServers() {
    try {
      ...
      this.outputChannel.appendLine(
        `[MCP]   - User: ${configs.user?.mcpServers ? Object.keys(configs.user.mcpServers).length : 0} servers`
      );
      this.outputChannel.appendLine(
        `[MCP]   - Project: ${configs.project?.mcpServers ? Object.keys(configs.project.mcpServers).length : 0} servers`
      );
      this.outputChannel.appendLine(
        `[MCP]   - Local: ${configs.local?.mcpServers ? Object.keys(configs.local.mcpServers).length : 0} servers`
      );
      ...
      servers.forEach((server) => {
        this.outputChannel.appendLine(
          `[MCP]   - ${server.name} (${server.scope}) - ${server.enabled ? 'enabled' : 'disabled'}`
        );
      });
      ...
      for (const server of servers) {
        if (server.enabled) {
          try {
            ...
            this.outputChannel.appendLine(
              `[MCP]   - ${server.name}: ${server.toolCount} tools, ${server.promptCount} prompts`
            );
          } catch (error) {
   ...
   * @returns Promise that resolves when hooks are configured
   */
