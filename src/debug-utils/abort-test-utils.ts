...
class AbortTestUtils {
    private context: AbortTestContext = {};
    ...
    async setup(): Promise<void> {
        ...
        try {
            ...
            this.context.processManager = serviceContainer.get('ClaudeProcessManager') as ClaudeProcessManager;
    ...
    async startProcess(): Promise<void> {
        ...
        try {
            ...
            if (result.ok) {
                ...
                result.value.on('exit', (code: number, signal: string) => {
                    console.log('\n📤 Process Exit Event:');
                ...
                // Monitor stdout
                result.value.stdout.on('data', (data: Buffer) => {
...
// Export to global for Debug Console access
declare global {
    var abortTest: AbortTestUtils;
}
